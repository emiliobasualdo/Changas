...
public class InscriptionJdbcDaoTest {
    ...
    private static final long USER3_ID = 333;
    ...
    @Test
    public void testIsUserInscribedInChanga_returnsUserNotInscribed() {
        //SETUP
        //Make sure the user owner of the changa is in the DB
        Number userOwnerId =  addUserToDatabase(USER1_EMAIL, PASSWORD);
       ...
       // when(inscriptionDao.getInscription(userNotInscribed.longValue(), changaId.longValue())).thenReturn(Either.alternative(USER_NOT_INSCRIBED));
        ...
        //ASSERT
        //assertNotNull(eitherIsUserInscribedInChanga);
        //assertFalse(eitherIsUserInscribedInChanga.isValuePresent());
        assertEquals(USER_NOT_INSCRIBED , eitherIsUserInscribedInChanga.getAlternative().getEc());
    }
    ...
    public void testGetUserInscriptions_returnsUserChangasInscriptions() {
        ...
        Inscription inscripiton1 = new Inscription.Builder().withUserId(userInscribedId.longValue()).withChangaId(changa1Id.longValue()).build();
        addInscriptionToDataBase(userInscribedId.longValue(), changa2Id.longValue());
        Inscription inscripiton2 = new Inscription.Builder().withUserId(userInscribedId.longValue()).withChangaId(changa2Id.longValue()).build();
        addInscriptionToDataBase(userInscribedId.longValue(), changa3Id.longValue());
        Inscription inscripiton3 = new Inscription.Builder().withUserId(userInscribedId.longValue()).withChangaId(changa3Id.longValue()).build();
    ...
    public void testInscribeInChanga_returnsCorrectValidation() {
        ...
        // SETUP:
        //Preparamos al falso usuario dueño de la changa
//        User userOwner = Mockito.mock(User.class);
//        when(userOwner.getUser_id()).thenReturn(USER1_ID);
//        when(userOwner.getPasswd()).thenReturn(PASSWORD);
//        when(userOwner.getEmail()).thenReturn(USER1_EMAIL);
        //Preparamos al falso usuario que se inscribirá en la changa
//        User userToInscribe = Mockito.mock(User.class);
//        when(userToInscribe.getUser_id()).thenReturn(USER2_ID);
//        when(userToInscribe.getPasswd()).thenReturn(PASSWORD);
//        when(userToInscribe.getEmail()).thenReturn(USER2_EMAIL);
        //Preparamos a la falsa changa creada por USER1
        Changa changa = Mockito.mock(Changa.class);
        when(changa.getChanga_id()).thenReturn(CHANGA_ID);
        when(changa.getUser_id()).thenReturn(USER1_ID);
        when(changaDao.getById(CHANGA_ID)).thenReturn(Either.value(changa));

        //     when(inscriptionDao.getInscription(USER2_ID, CHANGA_ID)).thenReturn(Either.alternative(new Validation(USER_NOT_INSCRIBED)));

        inscriptionDao.banal(USER2_ID, CHANGA_ID);

        //EJERCITAR
        //       Validation val = inscriptionDao.inscribeInChanga(USER2_ID, CHANGA_ID);
//         ASSERT
        //       assertNotNull(val);
        //     assertEquals(OK, val.getEc());
